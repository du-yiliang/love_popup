...
class FloatingText:
    """漂浮文字类"""
    def __init__(self, canvas, text, x, y, font_size=14, screen_height=1080, color='#ff6b9d'):
        self.canvas = canvas
        self.text = text
        self.current_x = x
        self.current_y = y
    ...
    def show(self, delay=0):
        ...
        for alpha in range(0, 105, 8):
            if not self.is_alive:
                break
            a = min(1.0, alpha / 100)
            color = self._blend_color(self.color, a)
    ...
    def fall_down(self, speed=8, fade_start=0.3):
        ...
        while self.is_alive:
            ...
            if self.current_y > self.screen_height * fade_start:
                ...
                alpha = max(0, alpha)
                color = self._blend_color(self.color, alpha)
...
class HeartTextAnimation:
    ...
    def phase_heart_formation(self):
        ...
        for i, (text, point) in enumerate(zip(shuffled_blessings, heart_points)):
            ...
            ft = FloatingText(self.canvas, text, start_x, start_y, font_size=13,
                            screen_height=self.screen_height, color=color)
    ...
    def phase_scatter(self):
        ...
        for _ in range(25):
            ...
            color = random.choice(TEXT_COLORS)  # 随机颜色
